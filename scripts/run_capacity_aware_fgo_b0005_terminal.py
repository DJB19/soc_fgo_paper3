...
# Reduce voltage weight because the cycle-1 voltage model is not fully aging-aware.
w_volt = 5.0
...
for stage, cycle_id in selected_cycles.items():
    ...
    soc_ca_fgo = np.clip(result.x[:n], 0.0, 1.0)
    q_est = float(result.x[-1])
    soc_cc_nominal = soc_init

    ca_metrics = calc_metrics(soc_ca_fgo, soc_ref)
    cc_metrics = calc_metrics(soc_cc_nominal, soc_ref)

    for method, soc_est, metrics, q_est_value in [
        ("Terminal-Constrained Capacity-Aware FGO", soc_ca_fgo, ca_metrics, q_est),
    ...
    plt.plot(time, soc_ca_fgo, "-.", label="Terminal-Constrained CA-FGO")
    plt.xlabel("Time (s)")
    plt.ylabel("SOC")
    plt.title(f"B0005 {stage.capitalize()} Cycle {cycle_id}: Terminal-Constrained CA-FGO")
    plt.legend()
    plt.grid(True)
    plt.savefig(FIG_DIR / f"B0005_cycle{cycle_id}_terminal_ca_fgo_soc.png", dpi=300)
    plt.close()

    plt.figure()
    plt.plot(time, soc_cc_nominal - soc_ref, "--", label="Nominal-Capacity CC Error")
    plt.plot(time, soc_ca_fgo - soc_ref, "-.", label="Terminal-Constrained CA-FGO Error")
    plt.xlabel("Time (s)")
    plt.ylabel("SOC Error")
    plt.title(f"B0005 {stage.capitalize()} Cycle {cycle_id}: Terminal-Constrained CA-FGO Error")
    plt.legend()
    plt.grid(True)
    plt.savefig(FIG_DIR / f"B0005_cycle{cycle_id}_terminal_ca_fgo_error.png", dpi=300)
    plt.close()

    print(
        f"{stage} cycle {cycle_id}: "
        f"TCA-FGO RMSE={ca_metrics['rmse']:.6f}, "
        f"Nominal CC RMSE={cc_metrics['rmse']:.6f}, "
        f"Q_true={q_true:.6f}, Q_est={q_est:.6f}, "
        f"Q_err={q_est - q_true:.6f}, "
        f"success={result.success}, nfev={result.nfev}"
    )

metrics_df = pd.DataFrame(rows)
metrics_df.to_csv(RESULT_DIR / "B0005_terminal_capacity_aware_fgo_metrics.csv", index=False)

print("Saved: results/B0005_terminal_capacity_aware_fgo_metrics.csv")
